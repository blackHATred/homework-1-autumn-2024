...
- Если ввести строку, по которой нельзя определить фильм/сериал или персону, то поиск не выдаст результатов:   
  ...
  Пример: слишком короткая строка (1 буква)      
...
- При вводе большого числа символов в поисковую строку (40 000 символов) поиск отображает ошибку пользователю с сообщением как на скриншоте:  
...
> Дополнение к предыдущему: при вводе очень большого числа символов  (40 000 символов) поиск одновременно выводит строку об отсутствии результатов и показывает спиннер загрузки результатов. При повторном открытии поиска спиннер не исчезает.  
> 
...
- При нажатии на крестик вне поисковой строки поисковая строка сворачивается.  
...
- Страница подборок состоит из следующих компонентов:
  ...
  - Карточки подборок, отфильтрованные по выбранной категории. Карточки имеют:
...
- При нажатии на одну из категорий подборок сами подборки фильтруются в соответствии с выбранной категорией   

![image](https://github.com/user-attachments/assets/0fe4c228-16b8-4be9-8f7b-c1667e968cea)

- При нажатии на подборку открывается ее страница, состоящая из таких компонентов:
  - Название подборки
  - Список карточек фильмов, соответствующих подборке    
   
![image](https://github.com/user-attachments/assets/37ecb5cb-9691-4bcf-ade6-17f68fa927c1)  

- Информация на карточке фильма полностью соответствует с информацией на странице фильма.
...
- Страница календаря релизов состоит из следующих компонентов:
  ...
  - Список месяцев
  - Карточки с фильмами релиза, соответствующие выбранному месяцу. При отсутствии фильмов релиза выводится надпись "Нет ожидаемых релизов в указанном месяце"
...
- При выборе месяца в блоке с контентом отображаются все фильмы релиза этого месяца.   
...
- Страница фильма состоит из следующих компонентов:
  - Название фильма
  - Постер фильма и трейлер
  - Постер на заднем фоне (background)
  - Рейтинг киноскопа, рейтинг IMDB  (на странице фильма календаря релизов рейтинг отсутствует)
  - Кнопка "Оценить" фильм
  - Кнопка "добавить в избранное" (если пользователь вошел в аккаунт)
  - Информация о фильме (Оригинальное название, страна, жанры ... ) и персоны в главных ролях   
  - Описание фильма   
  - Кадры из фильма   
  - Интересные факты   
  - Похожие фильмы   
  - Отзывы на фильм (при наличии)
  - Форма для написания отзыва (если пользователь вошел в аккаунт)   
...
- При наведени на персону/ на жанр, он должен подсвечиваться фиолетовым   
![image](https://github.com/user-attachments/assets/d6befb47-dccf-46fc-8c71-fbab0507b107)

- При нажатии на персону со страницы фильма должна открыться страница соответствующей персоны.
- При нажатии на жанр со страницы фильма должна открыться страница подборки соответствующего жанра.
- Если у фильма больше 6 похожих фильмов, должен появляться слайдер:
...
- Если пользователь не авторизован, то он не может добавить контент в избранное, сам раздел "Избранное" у него отсутствует, а при входе в аккаунт - раздел появляется рядом с аватаркой пользователя.    
...
- Находясь в избранном, можно удалить оттуда фильм, нажав на красную иконку в углу карточки фильма.    
![image](https://github.com/user-attachments/assets/eed4bfb4-f404-4570-8b86-b1d12b0d85fc)          
