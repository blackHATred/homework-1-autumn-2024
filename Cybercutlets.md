...
- При открытии страницы подборок она имеет такую структуру: под хедером отображается заголовок "Подборки", ниже - категории, ниже - перечислены подборки выбранной категории.
...
- При открытии страницы календаря релизов она имеет такую структуру: под хедером отображается заголовок "Календарь", ниже - раскрывающийся список с выбором года. Далее слева в столбце выводятся месяцы, а справа - фильмы релиза этого месяца.

- По умолчанию при открытии календаря релизов он показывает текущий месяц.

- Если фильмов релиза в месяце нет, то выводится надпись "Нет ожидаемых релизов в указанном месяце"   
...
- При нажатии на кнопку добавления в избранное на странице фильма ее надпись меняется, также загорается звездочка:     
