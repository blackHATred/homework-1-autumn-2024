...
## Правила валидации<a id='validation'></a>
...
- Изображение аватара считается валидным, если имеет формат .jpeg, .png, .gif, размером до 10 МБ. При наличии альфа-канала, он заменяется однотонным цветом. При наличии нескольких кадров в изображении, берётся первый кадр.  
- Имя пользователя может содержать любые символы, но не может быть пустым или иметь в себе более 30 символов.
- Заголовок рецензии может содержать любые символы, но не может быть пустым или иметь в себе более 50 символов.
- Содержание рецензии может содержать любые символы, но не может быть пустым или иметь в себе более 10000 символов.

## Регистрация<a id='register'></a>
### Страница регистрации
- При вводе [валидных данных](#validation) и нажатии на кнопку "продолжить" пользователь производит регистрацию  
  <img width="429" alt="image" src="https://github.com/user-attachments/assets/6a235b63-6cfc-4dbb-b7dd-39eed4d50b9b">
- При удачной регистрации пользователь редиректится на главную страницу (https://kinoskop.online/)  
- При вводе [невалидного пароля](#validation) на странице регистрации показывается сообщение о причине невалидности  
...
## Авторизация<a id='auth'></a>
### Хедер и состояние авторизации<a id='header'></a>
...
- При вводе корректных данных на странице авторизации пользователь переходит на главную страницу (https://kinoskop.online/) и состояние страницы меняется (можно определить по хедеру, [см. пункт выше](#header))
...
- При нажатии на пункт меню "выход" состояние страницы меняется (можно определить по хедеру, [см. пункт выше](#header)) и пользователь выходит из аккаунта
...
- При нажатии на изображение профиля открывается системное меню выбора изображения. Если пользователь выбирает [валидное изображение](#validation), то появляется зеленая галочка, аватар в хедере обновляется.  
...
- При выборе [невалидного изображения](#validation) или проблем с интернетом / на сервере пользователь видит описание ошибки  
  <img width="363" alt="image" src="https://github.com/user-attachments/assets/d1cbdb95-1c47-49c9-9508-b3be54ceabd4">  
- Если введены [невалидные данные](#validation), то пользователь получает сообщение об ошибке  
  <img width="415" alt="image" src="https://github.com/user-attachments/assets/99bc115d-495a-4dc8-bce1-4c04dfc40ac7">  
- Если [данные валидны](#validation) и пользователь нажал "редактировать", то отображается характерная зеленая галочка  
...
- Поведение инпутов пароля совпадает с поведением на страницах [авторизации](#auth) и [регистрации](#register), т.е. работает переключение видимости пароля, производится проверка на валидность и надежность пароля  
- Если пользователь ввёл верный текущий пароль и [валидный](#validation) новый пароль дважды, то при нажатии на кнопку "обновить" происходит обновление пароля.
...
- При нажатии на звезду она сохраняет своё состояние, выбор пользователя запоминается.
...
- При [нарушении ограничений составления рецензии](#validation) появляется характерное сообщение об ошибке  
