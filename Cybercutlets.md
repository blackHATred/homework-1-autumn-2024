...
- Поиск состоит из следующих компонентов:
  - Поисковая строка
  - Иконка крестика внутри поисковой строки (появляется при вводе), который удаляет введенную строку
  - Иконка крестика вне поисковой строки, который закрывает поиск
  - Блок результатов, где перечислены подходящие карточки фильмов в разделе "Фильмы и сериалы" и персон в разделе "Персоны"
  - Карточка фильма содержит постер, название на русском, оригинальное название, жанр, рейтинг(если это не фильм календаря релиза)
  - Карточка персоны содержит изображение персоны, ее имя
...
- При вводе большого числа символов в поисковую строку текст обрезается до допустимой длины (150 символов).    
...
> БАГ: Отображение рейтинга "0.0" фильма календаря релиза в поиске.   
...
- При изменении масштаба страницы в подборках названия, которые выходят за границы страницы, завершаются многоточием:    
...
- Карточки фильмов на странице подборки состоят из следующих компонентов:
  - Постер фильма
  - Рейтинг фильма:
...
- По умолчанию при открытии календаря релизов он показывает текущий месяц текущего года. 

- При выборе месяца в блоке с контентом отображаются все фильмы релиза этого месяца выбранного года.   
...
- Страница фильма состоит из следующих компонентов:
  ...
  - Похожие фильмы   
  - [Рецензии](#рецензии) на фильм (может отсутствовать)    
